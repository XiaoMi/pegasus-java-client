...
public class PegasusClient implements PegasusClientInterface {
    ...
    public boolean multiGet(String tableName, byte[] hashKey,
        ...
        PegasusTable tb = getTable(tableName);
        PegasusTableInterface.MultiGetResult res = tb.multiGet(
                hashKey, startSortKey, stopSortKey, options, maxFetchCount, maxFetchSize, 0);
        for (Pair<byte[], byte[]> kv : res.values) {
            values.add(kv);
        }
        return res.allFetched;
    ...
    public PegasusTableInterface.CheckAndSetResult checkAndSet(String tableName, byte[] hashKey,
        ...
        PegasusTable tb = getTable(tableName);
        return tb.checkAndSet(hashKey, checkSortKey, checkType, checkOperand, setSortKey, setValue, options, 0);
    }
