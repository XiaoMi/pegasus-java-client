...
public class PegasusScanner implements PegasusScannerInterface {
    ...
    public PegasusScanner(Table table, gpid[] splitGpid, ScanOptions options, long[] splitHash, boolean needCheckHash) {
        this(table, splitGpid, options, min, max, splitHash, needCheckHash);
        options.startInclusive = true;
        options.stopInclusive = false;
    }

    public PegasusScanner(Table table, gpid[] splitGpid, ScanOptions options,
                          blob startKey, blob stopKey, long[] splitHash, boolean needCheckHash) {
        _table = table;
    ...
    public void close() {
        if (_context >= CONTEXT_ID_VALID_MIN) {
            try {
                rrdb_clear_scanner_operator op = new rrdb_clear_scanner_operator(_gpid, _table.getTableName(), _context, _hash);
                _table.operate(op, 0);
            } catch (Throwable e) {
                // ignore
            }
    ...
    private void asyncStartScan() {
        if (_rpcRunning) {
            logger.error("scan rpc already be running, encounter logic error, we just abandon this scan, " +
                    "tableName({}), appId({})", _table.getTableName(), _table.getAppID());
        ...
        rrdb_get_scanner_operator op = new rrdb_get_scanner_operator(_gpid, _table.getTableName(), request, _hash);
    ...
    private void onRecvRpcResponse(error_code err, scan_response response) {
        if (!_rpcRunning) {
            logger.error("scan rpc haven't been started, encounter logic error, we just abandon this scan, " +
                    "tableName({}), appId({})", _table.getTableName(), _table.getAppID());
            _encounterError = true;
        ...
        if (err.errno == error_code.error_types.ERR_OK) {
            ...
            else if (response.error == 1) { // rocksDB error kNotFound, that scan context has been removed
    ...
    private void asyncNextInternal() {
        ...
        while (!_promises.isEmpty()) {
            ...
            p.setSuccess(new ImmutablePair<Pair<byte[], byte[]>, byte[]>(
    ...
    final private Object _promisesLock = new Object();
