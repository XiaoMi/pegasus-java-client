...
public class ThriftFrameEncoder extends MessageToByteEncoder<ReplicaSession.RequestEntry> {
    ...
    protected void encode(ChannelHandlerContext ctx, ReplicaSession.RequestEntry e, ByteBuf out) throws Exception{
        ...
        out.setBytes(initIndex, e.op.prepare_thrift_header(out.readableBytes() - ThriftHeader.HEADER_LENGTH, (int)e.timeoutMs));
    }

    @Override
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        logger.warn("got exception in outbound handler of {}, just ignore this: ", ctx.channel().toString(), cause);
    }
}
