...
public class TestBasic {
  ...
  public void testScanRangeWithValueExpired() throws PException, InterruptedException {
    ...
    for (int i = 0; i <= 4; i++) {
      Assert.assertEquals("persistent_" + i, new String(caseD1.keys.get(i)));
    }
    ...
    for (int i = 0; i <= 9; i++) {
      Assert.assertEquals("persistent_" + i, new String(caseD2.keys.get(i)));
    }
  }
  ...
  private void assertScanResult(
    ...
    Assert.assertEquals(expectAllFetched, actuallyRes.allFetched);
    Assert.assertEquals(stopIndex - startIndex + 1, actuallyRes.results.size());
