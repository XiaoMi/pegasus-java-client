...
public class ReplicaSessionTest {
    ...
    public void testRecvInvalidData() throws Exception {
        ...
        for (int pid = 0; pid < 16; pid++) {
            ...
            update_request req = new update_request(new blob("a".getBytes()), new blob("a".getBytes()), 0);
            final client_operator op = new test_operator(new gpid(1, pid), req);
            FutureTask<Void> cb = new FutureTask<Void>(new Callable<Void>() {
                @Override
                public Void call() throws Exception {
                    if (op.rpc_error.errno != error_code.error_types.ERR_OBJECT_NOT_FOUND &&
                        ...
                        Assert.assertEquals(error_code.error_types.ERR_INVALID_DATA, op.rpc_error.errno);
                    }
                    return null;
                }
            });
